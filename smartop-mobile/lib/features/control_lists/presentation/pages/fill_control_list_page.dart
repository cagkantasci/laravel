...
class FillControlListPage extends StatefulWidget {
  ...
  const FillControlListPage({super.key, required this.controlList});
...
class _FillControlListPageState extends State<FillControlListPage> {
  ...
  Future<void> _startControlList() async {
    ...
    try {
      final updated = await _service.startControlList(
  ...
  Future<void> _updateItem(
    ...
    try {
      ...
      // Update local item
      final itemIndex = _controlList.items.indexWhere((i) => i.id == item.id);
      if (itemIndex != -1) {
        final updatedItem = item.copyWith(
          status: (updates['checked'] as bool?) == true
              ? 'completed'
              : 'pending',
          result: updates['value'] as String?,
          notes: updates['notes'] as String?,
        );

        final updatedItems = List<ControlItem>.from(_controlList.items);
  ...
  Future<void> _completeControlList() async {
    ...
    final requiredItems = _controlList.items.where((i) => i.isRequired);
    ...
    if (confirmed == true && mounted) {
      ...
      try {
        final updated = await _service.completeControlList(
          _controlList.id.toString(),
        );
        setState(() {
          _controlList = updated;
  ...
  Widget build(BuildContext context) {
    ...
    final canEdit =
        _hasStarted && !_controlList.isCompleted && !_controlList.isApproved;
              ...
              child: Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: _controlList.items.asMap().entries.map((entry) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_controlList.machineName.isNotEmpty) ...[
            Row(
              children: [
                Icon(
